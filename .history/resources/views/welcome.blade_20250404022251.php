...
const App = () => {
  ...
  // Handle scroll event to add the opacity and blur effect
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 0) {
        setIsScrolled(true);
  ...
  return (
    <div className="relative">
      <nav
        className={`px-6 py-4 flex justify-between items-center sticky top-0 transition-all duration-300 ease-in-out ${
          isScrolled ? 'bg-gray-800 bg-opacity-80 backdrop-blur-lg' : 'bg-transparent'
        }`}
      >
      ...
      {/* Mobile menu */}
      {showMenu && (
        <div className="md:hidden bg-gray-800 glass-card mt-2 rounded-lg p-4 mx-4 transition-all duration-300 ease-in-out">
